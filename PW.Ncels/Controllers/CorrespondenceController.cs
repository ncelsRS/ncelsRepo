...
namespace PW.Ncels.Controllers
{
    public class CorrespondenceController : ACommonController
    {
        ...
        public ActionResult Send(Document model) {
            ...
            //Если Ценообразование - пробуем отправить уведомление
            if (model.ApplicantType == 1) {
                var answerId = new Guid(model.AnswersId);
                var baseDocument = db.Documents.FirstOrDefault(x => x.Id == answerId);

                if (baseDocument != null) {

                    var registartor = db.Employees.FirstOrDefault(x => x.Id == new Guid(baseDocument.RegistratorId));
                    var executors = db.Tasks.Where(x => x.DocumentId == baseDocument.Id).Select(x => x.ExecutorId).ToList();
                    if (registartor != null) {
                        executors.Add(registartor.Id.ToString());
                    }
                    var dExecutors = executors.Distinct();
                    foreach (var executorId in dExecutors) {
                        if (!string.IsNullOrEmpty(executorId)) {
                            new NotificationManager().SendNotification(
                                string.Format("На заявление №{0} пришло письмо от заявителя",
                                    string.IsNullOrEmpty(baseDocument.OutgoingNumber)
                                        ? baseDocument.Number
                                        : baseDocument.OutgoingNumber),
                                ObjectType.Letter, doc.Id, new Guid(executorId));
                        }
