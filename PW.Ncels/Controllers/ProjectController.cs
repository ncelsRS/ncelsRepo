...
#region VIEWS
        ...
        [HttpPost]
        public async Task<JsonResult> ReadPriceList(ModelRequest request, int type){
            return Json(await ProjectServices.Instance.GetPpPriceList(db, request, type), JsonRequestBehavior.AllowGet);
        }

        public JsonResult LoadAttachRemarks(Guid id) {
            var list = db.PP_AttachRemarksView.Where(x => x.PriceProjectId == id);
        ...
        public ActionResult PriceLsSave(RequestModel model) {
            if (model != null && model.CurentPrice != null){
                model.CurentPrice.UnitPrice = Math.Round(model.CurentPrice.UnitPrice, 2);
            }
	    ...
	    private void CommonFillPriceProject(PriceProject project) {
            if(!project.RequestOrderYear.HasValue)
	            project.RequestOrderYear = DateTime.Now.Year + 1;

            var prId = project.Id.ToString();
            if (project.Status == (int)PriceProjectStatus.OnRevision){
                var baseDocument = db.Documents.Where(x => x.AnswersId == prId && x.TemplateId != null).OrderByDescending(x=>x.CreatedDate).FirstOrDefault();
                if (baseDocument != null) {
                    var remarkType = db.RemarkTypes.FirstOrDefault(x => x.Id == baseDocument.RemarkId);
                    if (remarkType != null) {
                        project.RemarkTypeCode = remarkType.Code;
                    }
                }
            }
        }
	    ...
	    [HttpGet]
        public ActionResult LoadPriceLs(Guid id){
            try {
                ...
                if (project == null){
                    ...
                    curentPrice = db.Prices.FirstOrDefault(o => o.Type == (int)PriceType.LsCurrentPrice && o.PriceProjectId == id);
                }

                CommonFillPriceProject(project);

                var response = Json(new RequestModel{
                    Project = project,
                ...
                LogHelper.Log.Error("Не удалось получить данные", ex);
                throw;
            }
        ...
        public JsonResult LoadPriceImn(Guid id) {
            ...
            if (project == null) {
                ...
                currentPrice = new Price{
                    Type = (int) PriceType.ImnCurrentPrice,
                    PriceProjectId = id,
                    UnitPriceCurrencyDicId = tengeDicId,
                    Id = Guid.NewGuid()
                };
            }
            else {
                ...
                currentPrice = db.Prices.FirstOrDefault(o => o.Type == (int) PriceType.ImnCurrentPrice && o.PriceProjectId == id && o.MtPartsId == null);
        ...
        public ActionResult PriceImnSaveNotCompleteness(RequestModel model) {
            ...
            if (project) {
                ...
                if (model.CurentPrice != null && model.CurentPrice.MtPartsId == null) {
        ...
        public JsonResult LoadRePriceImn(Guid id) {
            ...
            if (project == null) {
                ...
                proxyOrganization = db.Organizations.FirstOrDefault(o => o.Id == project.ProxyOrganizationId);
                manufacturerOrganization = db.Organizations.FirstOrDefault(o => o.Id == project.ManufacturerOrganizationId);
                currentPrice = db.Prices.FirstOrDefault(o => o.Type == (int)PriceType.ReImnCurrentPrice && o.PriceProjectId == id && o.MtPartsId == null);
                if (currentPrice == null){
                    currentPrice = new Price{
                        Type = (int)PriceType.ReImnCurrentPrice,
                        PriceProjectId = id,
        ...
        public ActionResult RePriceLs(Guid? id,Guid? parentId) {
            ...
            ViewBag.AllowReason = true;
            if (id.HasValue) {
                var pp = db.PriceProjects.FirstOrDefault(o => o.Id == id);
                if (pp != null && !pp.PriceProjectId.HasValue) {
                    ViewBag.AllowReason = false;
                }
            }
        ...
        public JsonResult LoadRePriceLs(Guid id) {
            ...
            if (project == null) {
                ...
                proxyOrganization = db.Organizations.FirstOrDefault(o => o.Id == project.ProxyOrganizationId);
                manufacturerOrganization = db.Organizations.FirstOrDefault(o => o.Id == project.ManufacturerOrganizationId);
                curentPrice = db.Prices.FirstOrDefault(o => o.Type == (int)PriceType.ReLsCurrentPrice && o.PriceProjectId == id);
            }
