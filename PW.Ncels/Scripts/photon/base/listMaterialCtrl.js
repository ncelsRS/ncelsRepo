var MaterialBase = {
    ...
    materialGridController: function ($scope, DTColumnBuilder, DTOptionsBuilder, $http) {
        $scope.dtColumns = [
            DTColumnBuilder.newColumn("IsAdditionalStr", "Дополнительно").withOption('name', 'IsAdditionalStr'),
...
//                });
        ...
        // remove
        $scope.remove = function (drugDeclarationId) {
            $http({
                url: '/Material/RemoveMaterial?id=' + MaterialBase.currentRowId + '&drugDeclarationId=' + drugDeclarationId,
                method: 'POST'
            }).success(function (response) {
                location.href = '/DrugDeclaration/Edit?id=' + drugDeclarationId + '&currentStage=materialAttachBtn';
            });
        }
    },
    materialFormController: function ($scope, $http, $uibModal) {
        ...
        $scope.ReTestDateBlockVisible = false;
        $scope.drugFormVisibility = true;
        $scope.dosageVisibility = true;
        $scope.dosageUnitVisibility = true;
        $scope.dosageQuantityVisibility = true;
        $scope.setVisibilityByMaterialType = function (selectedItem) {
            debugger;
            $scope.ReTestDateBlockVisible = selectedItem.Code != "mpsample";
            if (selectedItem.Code === 'specificreagent' ||
                selectedItem.Code === 'standartsample' ||
                selectedItem.Code === 'consumables') {
                $scope.drugFormVisibility = false;
                $scope.dosageVisibility = false;
                $scope.dosageUnitVisibility = false;
                $scope.dosageQuantityVisibility = false;
            } else {
                $scope.drugFormVisibility = true;
                $scope.dosageVisibility = true;
                $scope.dosageUnitVisibility = true;
        ...
        $scope.saveMaterial = function () {
            if ($scope.materialForm.$valid) {
                $http({
                    ...
                    location.href = '/DrugDeclaration/Edit?id=' + $('#drugdeclarationId').val() + '&currentStage=materialAttachBtn';
                });
            } else {
