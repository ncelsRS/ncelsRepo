...
function IsLoadedDoc($http, $scope, $uibModal) {
    $http({
        ...
        if (result == "True") {
            ...
            customWarning("Требуются вложения:<br/> «Доверенность или копия» и «Сопроводительная письмо» и «Цена производителя в стране происхождения»");
        }
    });
}
...
function checkRemarkType($scope, $http, id) {
    var remarkTypeCode = $scope.object.Project.RemarkTypeCode;
    if (remarkTypeCode != null && remarkTypeCode === '3') {
        $("#tab-1").addClass("disabledbutton");
        $("#tab-2").addClass("disabledbutton");
        $("#tab-3").addClass("disabledbutton");
        $("#tab-4").addClass("disabledbutton");
        $http({
            method: "GET",
            url: "/Project/LoadAttachRemarks?id=" + id,
            data: "JSON"
        }).success(function (response) {
            $scope.attachRemarks = response;
            $("#attachRemarks").show();
        });
    }
}
...
function priceLsGrid($scope, DTColumnBuilder, $http, $uibModal) {
    ...
    $scope.SetObjectReg = function (isOrphan) {
        if (isOrphan) {
            ...
            $scope.imDisabled = $scope.curentReg.umId != null;
    ...
    $http({
        ...
        if (response.Project.RegNumber != null) {
            $scope.imDisabled = $scope.object.Project.IntroducingMethodDicId != null;
            $scope.isEnableDownload = true;
            $scope.isProjectSaved = true;
...
function priceImnGrid($scope, DTColumnBuilder, $http, $uibModal) {
    ...
    $scope.cbIsNameRuChangedEvent = function (isNameRuChanged) {
        if (isNameRuChanged) {
            $scope.object.Project.NameChangedRu = $scope.object.Project.NameRu;
        } else {
            $scope.object.Project.NameChangedRu = '';
        }
    }
    ...
    $scope.SetObjectReg = function() {
        //Если еще не пришли данные
        if ($scope.object.ManufacturerOrganization == null) {
            console.log('wait...');
            return;
        }
    ...
    $http({
        ...
        loadDictionaryParts($scope, $http, function() {
            $scope.IsCompleteness = false;

            if ($scope.object.CurentPrice != null
                && $scope.object.CurentPrice.MtPartsId != null) {
...
function rePriceImnNewGrid($scope, DTColumnBuilder, $http, $uibModal) {
    ...
    $http({
        ...
        $scope.addPrice();
        $scope.addCurrentPrice();
        loadDictionaryParts($scope, $http, null);
