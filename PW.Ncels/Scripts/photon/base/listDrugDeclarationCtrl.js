...
function actionsDeclarationHtmlAction(data, type, full, meta) {    
    ...
    if (full.StatusId === 1) {
        deleteBtn = "<li class=\"btn-danger\"><a href=\"#\" onclick=\"deleteApp('" + full.Id + "');\">" +
    ...
    return '<div class="btn-group" style="float: right;margin-right: 40px">' +
        ...
        '<li class="btn-default"><a href="/DrugDeclaration/DublicateDrug?id=' +
            full.Id +
            '" class="link-object" ><span class="glyphicon glyphicon-copy" aria-hidden="true"></span> Создать копию</a></li></ul></div>'
        ;
    ;
...
function drugDeclataionGrid($scope, DTColumnBuilder) {
    $scope.dtColumns = [
        ...
        DTColumnBuilder.newColumn("DrugNameRu", "Наименование препарата").withOption('name', 'DrugNameRu'),
