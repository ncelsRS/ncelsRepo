...
namespace Ncels.Core.ActivityManager
{
    public class ActivityManager
    {
        ...
        public List<EXP_Tasks> GetActiveTasks(Guid documentId, string activityTypeCode)
        {
            var tasks =
                _activityRepository.GetTasks(
                    t => t.EXP_Activities.DocumentId == documentId
                        && (t.EXP_Activities.ExpActivityStatus.Code == Dictionary.ExpActivityStatus.OnAgreement
                            || t.EXP_Activities.ExpActivityStatus.Code == Dictionary.ExpActivityStatus.Executed)
                        && t.EXP_Activities.ExpActivityType.Code == activityTypeCode).ToList();
            return tasks;
        }
