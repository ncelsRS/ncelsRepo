...
SELECT   
	...
	, sr_dos_f.name as _dosage_form_name
	, sr_dos_f.name_kz as _dosage_form_name_kz
	...
	, d_mt.id as degree_risk_id
FROM            
    dbo.sr_register AS sr_r 
    LEFT JOIN sr_register_drugs AS sr_r_d ON sr_r_d.id = sr_r.id 
	LEFT JOIN sr_international_names as sr_i_n on sr_r_d.int_name_id=sr_i_n.id
	LEFT JOIN sr_atc_codes AS sr_a_c ON sr_r_d.atc_id = sr_a_c.id 
	LEFT JOIN sr_dosage_forms as sr_dos_f ON sr_r_d.dosage_form_id = sr_dos_f.id
	...
	LEFT JOIN sr_countries as sr_p_c on sr_r_p.country_id=sr_p_c.id
	LEFT JOIN dbo.Dictionaries AS dic_c ON sr_p_c.name = dic_c.Name 
	...
	LEFT JOIN sr_register_mt as sr_mt ON sr_r.id = sr_mt.id
	LEFT JOIN Dictionaries as d_mt ON d_mt.Type='ImnSecuryType' AND sr_mt.degree_risk_id = d_mt.Code
WHERE        
	(sr_r.expiration_date > GETDATE() or sr_r.expiration_date is null) 
	--AND (
	...
	--)
	and (sr_r.reg_type_id=2 or sr_r_um.use_method_id is not null)
