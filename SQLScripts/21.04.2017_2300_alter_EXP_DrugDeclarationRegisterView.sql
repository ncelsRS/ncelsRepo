...
ds.DosageRu, ds.DosageKz, exs.Id as StageId, stage.NameRu as StageRu, stage.NameKz as StageKz, stages.Code StatusCode,
emp.Id as ExpertId, emp.ShortName as ExpertInitials, convert(bit, null) Suspended, convert(bit, 0) as Paid, convert(datetime, null) PaymentDate
/*convert(bit,iif(dp.Id is null,0,1)) as Paid, dp.PaymentDatetime1C as PaymentDate*/, convert(bit, null) OnBoard, convert(bit, null) ProductionEvaluation,
exs.StartDate StageStartDate, exs.EndDate StageControlDate, convert(datetime, null) ConclusionDate, convert(bit,IIF(stages.Code='4',1,0)) StageCompleted,
exs.FactEndDate StageEndDate, convert(bit, null) StageOverdue, convert(int, null) OverdueDays, convert(uniqueidentifier, null) LetterId,
...
--left join EXP_DirectionToPays_DrugDeclaration dpd on dpd.DirectionToPayId=dp.Id
--left join Dictionaries dps on dps.Id=dp.StatusId and dps.Code='6') dp on dp.DrugDeclarationId=d.Id
