...
AS
SELECT        Tdp.[Id], Tdp.[Number], Tdp.[CreateDate], Tdp.[DirectionDate] AS [DirectionDate], STUFF
                             ...
                             ((SELECT DISTINCT '; ' + Tt.DrugFormRu
                                 FROM            [dbo].[EXP_DrugDeclaration] Tt INNER JOIN
                                                          [dbo].[EXP_DirectionToPays_DrugDeclaration] AS Td ON Td.DrugDeclarationId = Tt.Id
                                 WHERE        Td.DirectionToPayId = Tdp.Id FOR XML PATH('')), 1, 2, '') AS DrugFormRu, '' AS DrugFormEn, '' AS ManufactureKz, '' AS ManufactureRu, '' AS ManufactureEn, '' AS ExpertDisplayName, CAST(0 AS decimal(18, 2)) 
                         AS Price, [TotalPrice] AS [TotalPrice], '' AS [Currency], STUFF
                             ((SELECT DISTINCT '; ' + Tt.NameKz
                             ...
                             ((SELECT DISTINCT '; ' + Tt.NameEn
                                 FROM            [dbo].[EXP_PriceList] Tt INNER JOIN
                                                          [dbo].[EXP_DirectionToPays_PriceList] AS Td ON Td.PriceListId = Tt.Id
                                 WHERE        Td.DirectionToPayId = Tdp.Id FOR XML PATH('')), 1, 2, '') AS PriceListNameEn, '' AS PriceListValue, [InvoiceNumber1C], [InvoiceDatetime1C], [PaymentDatetime1C], [PaymentValue1C], [PaymentComment1C], 
                         Tdp.[StatusId], Ts .[Code] AS StatusCode, Ts .[Name] AS StatusName, Ts .[NameKz] AS StatusNameKz, Tdp.CreateEmployeeId, Ta.TaskExecutorId AS ExecutorId, Ta.TaskExecutorValue AS ExecutorName, 
                         Ta.TaskComment AS Comment, Tdp.[PageCount], Tdp.[Type]
...
GROUP BY Tdp.Id, Tdp.[Number], Tdp.[DirectionDate], Tdp.[TotalPrice], Tdp.[InvoiceNumber1C], Tdp.[InvoiceDatetime1C], Tdp.[PaymentDatetime1C], Tdp.[PaymentValue1C], Tdp.[PaymentComment1C], Tdp.[StatusId], Ts .[Name], 
                         Ts .[NameKz], Ts .[Code], Tdp.CreateEmployeeId, Ta.TaskExecutorId, Ta.TaskExecutorValue, Ta.TaskComment, Tdp.[CreateDate], Tdp.[PageCount], Tdp.[Type]
