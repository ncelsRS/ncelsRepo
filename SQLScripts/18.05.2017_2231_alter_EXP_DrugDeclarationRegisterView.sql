alter view EXP_DrugDeclarationRegisterView as
SELECT
	d.Id, 
	d .Number, 
	d .TypeId, 
    ...
    case when exs.IsSuspended=0 
		then NULL 
		else DATEDIFF(dd,exs.SuspendedStartDate, CURRENT_TIMESTAMP)
	end as SuspensionPeriod,
	...
	convert(bit,iif(dp.Id is not null and dp.StatusCode='6',1,0)) as Paid,
	dp.PaymentDatetime1C as PaymentDate,
    convert(bit,iif(dp.Id is not null and dp.StatusCode='7',1,0)) as PaymentOverdue,
	dp.Number as PaymentNumber, dp.InvoiceNumber1C
FROM            
	...
	left join (select dpd. DrugDeclarationId, dps.Code as StatusCode, dp.* from EXP_DirectionToPays_DrugDeclaration dpd
    left join EXP_DirectionToPays dp on dp.Id=dpd.DirectionToPayId
	left join Dictionaries dps on dps.Id=dp.StatusId where dp.Type=1 and dp.DeleteDate is null) dp on dp.DrugDeclarationId=d.Id
where 
	exs.IsHistory=0
