...
ADD [IsUnitPrice] [bit] NOT NULL default(0)
GO
ALTER TABLE [dbo].[Prices]
ADD [IsManufacturerPrice] [bit] NOT NULL default(0)
GO
ALTER TABLE [dbo].[Prices]
ADD [IsLimitPrice] [bit] NOT NULL default(0)
GO
ALTER TABLE [dbo].[Prices]
ADD [IsAvgOptPrice] [bit] NOT NULL default(0)
GO
ALTER TABLE [dbo].[Prices]
ADD [IsAvgRozPrice] [bit] NOT NULL default(0)
GO
...
SELECT        p.Id, p.Type, p.Name, p.PriceProjectId, p.CountryId, p.ManufacturerPrice, p.ManufacturerPriceCurrencyDicId, p.ManufacturerPriceNote, p.LimitPrice, p.LimitPriceCurrencyDicId, p.LimitPriceNote, p.AvgOptPrice, 
                         ...
                         p.UnitPriceCurrencyDicId, d10.Name AS UnitPriceCurrencyName, p.CreatedDate, p.BritishCost, p.MtPartsId, sr_mt_p.name AS PartsName, p.IsIncluded, p.IsUnitPrice, p.IsAvgOptPrice, p.IsAvgRozPrice, 
                         p.IsLimitPrice, p.IsManufacturerPrice
