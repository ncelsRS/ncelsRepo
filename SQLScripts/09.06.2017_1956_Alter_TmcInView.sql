...
SELECT ti.Id,
       ...
       CASE
           WHEN DATEADD(DAY, 15, ti.CreatedDate) < GETDATE()
                AND ti.StateType < 2 THEN - 1
		   WHEN ti.StateType = 3  OR ti.StateType = 4 THEN ti.StateType
		   WHEN ti.StateType = 2  OR (Tla.PowerOfAttorneyNumber_1C IS NOT NULL) THEN 2 
           ELSE ti.StateType
       END AS StateType,
       CASE
           WHEN (DATEADD(DAY, 15, ti.CreatedDate) < GETDATE()
                 AND ti.StateType < 2)
                OR (ti.StateType = - 1) THEN N'Отклонена'
