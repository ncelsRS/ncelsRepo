...
/****** Object:  Trigger [dbo].[TR_SendToI1cOnUpdate]    Script Date: 14.05.2017 15:51:46 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	IF ((select [Type] from deleted) = 1)
	BEGIN
		-- Insert primary direction to pay
		IF (UPDATE(StatusId) AND (select StatusId from deleted) = @agreedStatusId 
				AND (select StatusId from inserted) = @sended1CStatusId)
		BEGIN
			...
			SELECT	NEWID() AS [Id]
					...
					,(CASE WHEN TOrf.Code = 'fl' THEN 1 ELSE 0 END) AS [IsLegal]
		...
		IF (UPDATE(StatusId) AND (select StatusId from deleted) IS NOT NULL 
				AND (select StatusId from inserted) = @expireStatusId)
		BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			FROM [dbo].[EXP_DirectionToPays] AS Td
