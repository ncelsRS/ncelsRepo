...
SELECT        pp.Id, pp.Type, (CASE WHEN doc.Number IS NOT NULL THEN doc.Number ELSE 'б/н' END) AS Number, (CASE WHEN doc.OutgoingNumber IS NOT NULL THEN doc.OutgoingNumber ELSE 'б/н' END) 
                         AS OutgoingNumber, pp.CreatedDate, doc.OutgoingDate, pp.Status, pp.OwnerId, pp.ManufacturerOrganizationId, pp.HolderOrganizationId, pp.ProxyOrganizationId, pp.DoverennostNumber, 
                         pp.DoverennostCreatedDate, pp.DoverennostExpiryDate, pp.Filial, pp.NameKz, pp.NameRu, pp.RegNumber, pp.RegDate, pp.LsTypeDicId, pp.NameOriginal, pp.MnnRu, pp.MnnEn, pp.FormNameKz, 
                         pp.FormNameRu, pp.Dosage, pp.CountPackage, pp.Concentration, pp.CodeAtx, pp.IntroducingMethodDicId, pp.IsConvention, pp.ImnSecuryTypeDicId, pp.RePriceDicId, pp.ResultTypeDicId, pp.IsPayed, 
                         pp.PayDate, pp.StartDate, pp.ContrDate, pp.ConclusionDate, pp.IsStageExpired, pp.ExpiredDayCount, pp.ExpertAz, pp.OutgoingDoc, pp.DayCount, pp.IsNewManufacrurer, mo.NameRu AS ManufacturerOrgName, 
                         po.NameRu AS ApplicantOrgName, d1.Name AS CountryName, pp.ListTypeDicId, pp.MnnOrderNumber, d2.Name AS RePriceName, 
