...
namespace PW.Ncels.Database.Controller
{
    public abstract class ADrugDeclarationController : System.Web.Mvc.Controller
    {
        ...
        protected void FillViewBag(EXP_DrugDeclaration model)
        {
            ...
            ViewData["AppPeriodOpenMeasureList"] = new SelectList(measures, "id", "short_name",
                null);
            if (model.SrRegisters == null)
            {
                model.SrRegisters = new List<sr_register>();
            }
            ViewData["RegNumberList"] = new SelectList(model.SrRegisters, "id", "reg_number",
                null);
            ...
            for (var i = 0; i < model.ExpDrugDosages.Count; i++)
            {
                ...
                ViewData["RegNumberList" + dosage.Id] = new SelectList(model.SrRegisters, "id", "reg_number",
                    model.ExpDrugDosages[i].RegisterId);
    ...
    */
        ...
        protected void FillDeclarationControl(EXP_DrugDeclaration model)
        {
            ...
            var dictionary = new Dictionary<int, sr_register>();
        
            foreach (var record in expDrugDosages)
            {
                if (record.RegisterId > 0 && record.sr_register != null)
                {
                    if (!dictionary.ContainsKey(record.RegisterId.Value))
                    {
                        dictionary.Add(record.RegisterId.Value,record.sr_register);
                    }
                    ...
                    if (!model.IsShowChange)
                    {
                        if (record.sr_register.expiration_date != null)
                        {
                            ...
                            if (model.TypeId > 1)
                            {
                                ...
                                else
                                {
                                    ...
                                    var t = record.sr_register.expiration_date.Value - model.CreatedDate;
                                    var month = t.TotalDays;
                        ...
                        else if ( record.sr_register.expiration_date == null)
                        {
                            model.IsShowChange = true;
                        }
                    }
                    ...
                    for (var d = 0; d < record.EXP_DrugSubstance.Count; d++)
                    {
                        ...
                        record.EXP_DrugSubstance.ToList()[d].ExpDrugSubstanceManufactures =
                            new List<EXP_DrugSubstanceManufacture>();
                        foreach (
                            var expDrugSubstanceManufacture in
                            record.EXP_DrugSubstance.ToList()[d].EXP_DrugSubstanceManufacture)
                        {
                            record.EXP_DrugSubstance.ToList()[d].ExpDrugSubstanceManufactures.Add(
                                expDrugSubstanceManufacture);
                        }
                    }
                    ...
                    if (model.StatusId > 6)
                    {
                        model.ConclusionSafetyReports.Add(
                            new SafetyreportRepository().ConclusionSafetyReportsFromDosage(record.Id, false));
                        model.ConclusionSafetyReports.Add(
                            new SafetyreportRepository().ConclusionSafetyReportsFromDosage(record.Id, true));
                    }
            }
         
            foreach (var srRegister in dictionary.Values)
            {
                model.SrRegisters.Add(srRegister);
            }
        ...
        [HttpPost]
        public virtual ActionResult SetMathValue(string id, string math)
        {
            new DrugDeclarationRepository().SetMathValue(long.Parse(id), math);
            return Json(new
            {
                isSuccess = true
            });
