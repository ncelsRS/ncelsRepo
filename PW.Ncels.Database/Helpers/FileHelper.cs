...
namespace PW.Ncels.Database.Helpers
{
    public static class FileHelper
    {
		...
		public static FileInfo GetProtocolAttachFile(ncelsEntities db, Guid protocolId) {
            try {
                ...
                var dirPath = Path.Combine(PathRoot, Root, protocolId.ToString());
  ...
  /*          }
        ...
        public static IEnumerable GetAttachListWithCodeEdit(ncelsEntities db, string doc, string type, bool byMetadata = false, string excludeCodes = null, bool isShowComment = false)
        {
            /* try
             {*/
            DirectoryInfo info = new DirectoryInfo(Path.Combine(ConfigurationManager.AppSettings["AttachPath"], Root, doc ?? ""));
