...
namespace PW.Ncels.Database.Repository.Price{
    ...
    public class PriceProjectRepository : ARepository{
        ...
        public DataModel.Price GetCurrentPrice(Guid Id) {
            var pp = AppContext.PriceProjects.FirstOrDefault(x => x.Id == Id);
            if (pp != null){
                DataModel.Price currentPrice = null;
                //Рассматриваем ЛС
                if (pp.Type == (int)PriceProjectType.PriceLs){
                    currentPrice = AppContext.Prices.FirstOrDefault(x =>
                        x.PriceProjectId == Id 
        ...
        /// <summary>
        /// Проверка срока ответа от внешнего заявителя
        /// </summary>
        public void CheckRefPassedDays() {
            LogHelper.Log.Debug("Старт проверки срока ответа от внешнего заявителя");
            var rows = AppContext.PriceProjectJournals.Where(x => 
                x.PassedDays > 10
                && x.Status == (int)PriceProjectStatus.OnRevision
            );
            if (!rows.Any())
                return;

            foreach (var row in rows) {
                LogHelper.Log.DebugFormat("rowId: {0}", row.Id);
                Document d = AppContext.Documents.FirstOrDefault(m => m.Id == row.Id);
                if (d != null){
                    d.IsArchive = true;
                    var pp = AppContext.PriceProjects.FirstOrDefault(x => x.Id == row.Id);
                    if (pp != null){
                        pp.Status = (int)PriceProjectStatus.Rejected;
                    }
                }
            }
            AppContext.SaveChanges();
            LogHelper.Log.Debug("Проверка срока ответа от внешнего заявителя завершена");
        }

        public void SendAvgRateNotification() {
            LogHelper.Log.Debug("CheckAvgRateNotification START");
            var today = DateTime.Now;
            var lastDay = DateTime.DaysInMonth(today.Year, today.Month)-2;
            //Шлем уведомление за 2 дня до конца месяца
            if (today.Day == lastDay) {
                var userList = AppContext.PP_DIC_AvgExchangeRate
                    .Where(x => x.ChangeEmployeeId != null)
                    .Select(x => x.ChangeEmployeeId)
                    .Distinct();
                foreach (var userId in userList) {
                    if (userId.HasValue) {
                        new NotificationManager().SendNotificationAnonymous("Не забудьте заполнить данные по средневшененным курсам валют", ObjectType.Letter, null, userId.Value);
                    }
                    
                }
            }

            LogHelper.Log.Debug("CheckAvgRateNotification END");
        }
