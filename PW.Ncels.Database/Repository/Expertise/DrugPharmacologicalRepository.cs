...
{
    public class DrugPharmacologicalRepository : ADrugDeclarationRepository
    {
        public void CloneDosageFinalDoc(Guid dosageId)
        {
            ...
            var currentFinal = dosage.EXP_ExpertisePharmacologicalFinalDoc.FirstOrDefault();
            ...
            {
                var expFinal = expDrugDosage.EXP_ExpertisePharmacologicalFinalDoc.FirstOrDefault() ??
                               new EXP_ExpertisePharmacologicalFinalDoc(currentFinal);
                expFinal.DosageStageId = expDrugDosage.Id;
                if (expFinal.Id == Guid.Empty)
                {
                    expFinal.Id = Guid.NewGuid();
                    AppContext.EXP_ExpertisePharmacologicalFinalDoc.Add(expFinal);
                }
            }
        ...
        public EXP_ExpertisePharmacologicalFinalDoc UpdateFinalDocument(string fieldName, string fieldValue, Guid objectId, Guid userId)
        {
            ...
            var model = appDosage.EXP_ExpertisePharmacologicalFinalDoc.FirstOrDefault() ??
                        new EXP_ExpertisePharmacologicalFinalDoc { DosageStageId = appDosage.Id };
