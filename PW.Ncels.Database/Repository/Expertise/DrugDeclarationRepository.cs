...
namespace PW.Ncels.Database.Repository.Expertise
{
            ...
            string fieldValue, string fieldDisplay)
        {
            ...
            switch (code)
            {
                ...
                case CodeConstManager.EXP_DrugOtherCountry:
                    {
                        return UpdateOtherCountry(code, model, recordId, fieldName, fieldValue, userId, fieldDisplay);
                    }
    ...
    */
        ...
        private SubUpdateField UpdateMain(bool isNew, EXP_DrugDeclaration model, string fieldName, string fieldValue, string userId,
            string fieldDisplay)
        {
            if (fieldName == "TypeId" && string.IsNullOrEmpty(fieldValue))
            {
                var subUpdateField1 = new SubUpdateField {ModelId = model.ObjectId};
                return subUpdateField1;
            }
        ...
        {
            var org = AppContext.EXP_DrugOrganizations.FirstOrDefault(e => e.Id == recordId);
            if (org == null)
            {
                return null;
            }
            var contractId = org.EXP_DrugDeclaration.ContractId;
        ...
        public virtual EXP_DrugDeclaration SaveOrUpdate(EXP_DrugDeclaration entity, Guid? userId)
        {
            ...
            var markList = GetPrimaryMarkList(entity.Id, null);
            foreach (var expExpertiseStageRemark in markList)
            {
                if (expExpertiseStageRemark.IsFixed && expExpertiseStageRemark.FixedDate==null)
                {
                    expExpertiseStageRemark.FixedDate = DateTime.Now;
                }
            }
        ...
        public DictionaryInfo GenerateNd(string modelId, string userId, int? typeId)
        {
            var model = GetById(modelId);
            var number = GetNdNumber(model, typeId);
            UpdatePrimaryNTD(model, "nt_0_NumberNd", number, userId);
            var dic = new DictionaryInfo();
            dic.NameRu = number;
            dic.TypeRu = model.TypeId.ToString();
            return dic;
        }
        ...
        public string GetNdNumber(EXP_DrugDeclaration model, int? typeId)
        {
            ...
            var ntd = AppContext.EXP_DrugPrimaryNTD.FirstOrDefault(e => e.DrugDeclarationId == model.Id);
            if (!string.IsNullOrEmpty(ntd?.NumberNd))
            {
                return ntd.NumberNd;
            }
        ...
        public string GetMathValue(long id)
        {
            var substance = AppContext.EXP_DrugSubstance.FirstOrDefault(e => e.Id == id);
            if (substance == null)
            {
                return "";
            }
            return substance.MathFormula;
        }

        public void SetMathValue(long id, string math)
        {
            var substance = AppContext.EXP_DrugSubstance.FirstOrDefault(e => e.Id == id);
            if (substance == null)
            {
             ...
             substance.MathFormula = math;
            AppContext.SaveChanges();
        }
        private SubUpdateField UpdateOtherCountry(string code, EXP_DrugDeclaration model, long? recordId,
       string fieldName, string fieldValue, string userId, string fieldDisplay) 
        {
            EXP_DrugOtherCountry entity = null;
            if (recordId > 0)
            {
                entity = AppContext.Set<EXP_DrugOtherCountry>().FirstOrDefault(e => e.Id == recordId);
            }
            if (entity == null)
            {
                entity = new EXP_DrugOtherCountry { EXP_DrugDeclaration = model };
            }
            var columnName = fieldName.Split('_')[2];
            var property = entity.GetType().GetProperty(columnName);

            if (columnName == "IsUnLimited")
            {
            ...
            if (entity.Id == 0)
            {
                AppContext.Set<EXP_DrugOtherCountry>().Add(entity);
            }
            AppContext.SaveChanges();
            var idControl = code + "_" + entity.Id + "_" + columnName;
            SaveHistoryField(model.Id, idControl, fieldValue, new Guid(userId), fieldDisplay);
