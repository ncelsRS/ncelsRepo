...
namespace PW.Ncels.Database.Repository.Expertise
{
    public class ExpertiseStageRepository : ARepository
    {
        ...
        {
            var stages = AppContext.EXP_ExpertiseStage.Include(e => e.EXP_ExpertiseStageExecutors).Where(e => stageIds.Contains(e.Id)).ToList();
            var executors = AppContext.Employees.Where(e => executorIds.Contains(e.Id)).ToList();
            foreach (var stage in stages)
            {
                stage.EXP_ExpertiseStageExecutors.AddRange(executors.Select(e=>new EXP_ExpertiseStageExecutors()
                {
                    Employee = e,
                    EXP_ExpertiseStage = stage
                }));
        ...
        public bool ToNextStage(Guid declarationId, Guid? fromStageId, int[] nextStageIds, out string resultDescription)
        {
            ...
            foreach (var nextStageId in nextStageIds)
            {
                ...
                var newStage = new EXP_ExpertiseStage(true)
                {
                    Id = Guid.NewGuid(),
                ...
                newStage.EXP_ExpertiseStageExecutors.AddRange(GetExecutorsByDicStageId(newStage.StageId, declaration.TypeId).Select(e=>
                    new EXP_ExpertiseStageExecutors()
                    {
                        Employee = e,
                        IsHead = true,
            ...
            if (isAnalitic)
            {
                //TODO Посмотреть Александру по поводу множества исполнителей
                CreateDirectionMaterial(declarationId, GetExecutorsByDicStageId(CodeConstManager.STAGE_ANALITIC, declaration.TypeId).FirstOrDefault());
            }
            return true;
        }
        ...
        private bool ToNextStage(EXP_DrugDeclaration declaration, Guid? fromStageId, int[] nextStageIds, out string resultDescription)
        {
            ...
            foreach (var nextStageId in nextStageIds)
            {
                var dicNextStage = GetDicStageById(nextStageId);
                if (!CanSendToStep(declaration.Id, fromStageId, nextStageId, out resultDescription)) return false;
                //если имеется уже выполняющийся этап то продолжаем его дальше
                if (dicNextStage.Code!=EXP_DIC_Stage.Translate && AppContext.EXP_ExpertiseStage.Any(e => e.DeclarationId == declaration.Id
                ...
                if (dicNextStage.Code == EXP_DIC_Stage.Translate || oldStage == null)
                {
                    var daysOnStage = GetExpStageDaysOnExecution(declaration.TypeId, nextStageId);
                    var startDate = DateTime.Now;
                    var newStage = new EXP_ExpertiseStage(true)
                    ...
                    //todo брать руководителя цоз из настроек
                    newStage.EXP_ExpertiseStageExecutors = new List<EXP_ExpertiseStageExecutors>();
                    newStage.EXP_ExpertiseStageExecutors.AddRange(GetExecutorsByDicStageId(newStage.StageId, declaration.TypeId).Select(e=>
                        new EXP_ExpertiseStageExecutors()
                        {
                            Employee = e,
                    ...
                    newStage.EXP_ExpertiseStageDosage = new List<EXP_ExpertiseStageDosage>();
                    foreach (var dosageId in dosageIds)
                    {
                        newStage.EXP_ExpertiseStageDosage.Add(new EXP_ExpertiseStageDosage()
                        {
                ...
                {
                    var reworkStage = new EXP_ExpertiseStage(true)
                    {
                        Id = Guid.NewGuid(),
                        DeclarationId = declaration.Id,
                        StageId = nextStageId,
                    ...
                    reworkStage.EXP_ExpertiseStageExecutors.AddRange(oldStage.EXP_ExpertiseStageExecutors.Select(e=>new EXP_ExpertiseStageExecutors()
                    {
                        Employee = e.Employee,
                        IsHead = e.IsHead,
        ...
        public bool ToBackStage(Guid declarationId, Guid? fromStageId, int[] nextStageIds, out string resultDescription)
        {
            ...
            foreach (var nextStageId in nextStageIds)
            {
                ...
                if (oldStage != null)
                {
                    ...
                    newStage.EXP_ExpertiseStageExecutors.AddRange(oldStage.EXP_ExpertiseStageExecutors.Select(e=>new EXP_ExpertiseStageExecutors()
                    {
                        Employee = e.Employee,
                        IsHead = e.IsHead,
                        EXP_ExpertiseStage = newStage
        ...
        /// <summary>
        /// Получение исполнителей этапа
        /// </summary>        
        /// <returns></returns>
        public List<Employee> GetExecutorsByDicStageId(int stageId, int regTypeId)
        {
        ...
        public Employee GetStageSupervisor(int stageId, int regTypeId)
        {
            return AppContext.EXP_RegistrationExpSteps
                .Where(e => e.RefId == stageId && e.EXP_RegistrationTypes.RefId == regTypeId)
                .Select(e => e.SupervisingEmployee).FirstOrDefault();
        }

        private int? GetExpStageDaysOnExecution(int expTypeId, int dicStageId)
        {            
            var registrationExpSteps =
                AppContext.EXP_RegistrationExpSteps.FirstOrDefault(e => e.EXP_RegistrationTypes.RefId == expTypeId && e.RefId==dicStageId);
        ...
        public int GetCountByStage(bool isNew, int stageId, string statusCode, Guid userId)
        {
            return AppContext.EXP_DrugDeclarationRegisterView.Count(e => e.IsNew == isNew && e.DicStageId == stageId && e.StatusCode==statusCode && e.ExpertId==userId);
        }

        public void SetStageViewved(Guid stageId)
        {
            var stage = AppContext.EXP_ExpertiseStage.Find(stageId);
            stage.IsNew = false;
            AppContext.SaveChanges();
        }

        public bool HasNotFixedRemarks(Guid stageId)
        {
            return AppContext.EXP_ExpertiseStageRemark.Any(e => e.StageId == stageId && (!e.IsFixed || !e.IsAccepted));
        }
