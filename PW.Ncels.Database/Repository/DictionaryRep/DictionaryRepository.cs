...
namespace PW.Ncels.Database.Repository.DictionaryRep
{
    public class DictionaryRepository : ARepositoryGeneric<DataModel.Dictionary>
    {
        ...
        public Guid? GetDictionaryIdByTypeAndName(string type, string name)
        {
            return AppContext.Dictionaries.Where(d => d.Type == type && d.Name == name).Select(d => d.Id).FirstOrDefault();
        }
    }
}