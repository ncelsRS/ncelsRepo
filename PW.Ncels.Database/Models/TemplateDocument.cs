...
namespace PW.Ncels.Database.Models
{
    public class TemplateDocument
    {
        public TemplateDocument(Document document)
        {
            ...
            var remarkType = entities.RemarkTypes.FirstOrDefault(x => x.Id == document.RemarkId);
            //Замечание по вложениям
            if (remarkType != null && remarkType.Code == "3") {
                var ppId = new Guid(document.AnswersId);
                var attachRemarks = entities.PP_AttachRemarksView.Where(x => x.PriceProjectId == ppId);
                if (attachRemarks.Any()) {
                    var i = 0;
                    foreach (var attachRemark in attachRemarks) {
                        i++;
                        ItemsValue["Comment"+i] = "Замечание по документу \"" + attachRemark.dicName + "\": " + attachRemark.RemarkNote;
                    }
                }
            }
        }
