...
namespace PW.Prism.Controllers
{
    ...
    public class TaskController : Controller
    {
        ...
        public ActionResult GetTasks([DataSourceRequest] DataSourceRequest request, int type)
        {
            ...
            bool isAddBp = EmployeePermissionHelper.IsAddBp;
            var tasks = queryable
                .Select(x => new TaskListItemModel
                {
                    ...
                    ExpertName = _db.Tasks.Where(t => t.Document.Id == x.Document.Id).OrderByDescending(t => t.CreatedDate).FirstOrDefault().ExecutorValue
                    //TypeValue = (x.Type == 2 || x.Type == 0) ? "Заявление" : "Письмо" // тип задания
            ...
            foreach (var taskListItemModel in tasks) {
                ...
                taskListItemModel.NameRu = DocumentHelper.GetTradeNameRu(_db, taskListItemModel.DocumentId);
        ...
        public ActionResult MultiReassignment(string modelid, string ids) {
            ViewBag.ParentModelId = modelid;
            if (string.IsNullOrEmpty(ids)) {
                ViewBag.ErrorMessage = "Необходимо выбрать хотя бы одну запись";
                return PartialView("ErrorMessageView");
            }
            var tasksId = ids.Split(',').Select(Guid.Parse);
            var tasks = _db.Tasks.Include(o => o.Document).Where(o => 
                tasksId.Contains(o.Id) 
                && o.Type == 0
                && (
                    o.State == 0
                    || o.State == 4
            ...
            if (!tasks.Any()) {
                ViewBag.ErrorMessage = "Среди выбранных записей нет задач на распределение";
                return PartialView("ErrorMessageView");
            }

            var model = new MultiReassignmentViewModel {
                Id = Guid.NewGuid(),
                TaskIds = string.Join(",", tasks.Select(t => t.Id.ToString()))
            };

            return PartialView(model);
        }

        public ActionResult MultiReassignmentConfirm(MultiReassignmentViewModel request) {
            try {
                if (string.IsNullOrEmpty(request.TaskIds)) {
                    return Json(new { State = false });
                }
                var tasksId = request.TaskIds.Split(',').Select(Guid.Parse);
                foreach (var taskId in tasksId) {
                    Task task = _db.Tasks.Include(o => o.Document).FirstOrDefault(o => o.Id == taskId);
                    if (task == null) {
                        throw new Exception("Не удалось найти таск по его идентификатору " + taskId);
                    }
                    Activity activity = new Activity{
                        Id = Guid.NewGuid(),
                        ParentTask = task.Id,
                        ...
                        ExecutorsId = DictionaryHelper.GetItemsId(request.ExecutorId),
                        ExecutorsValue = DictionaryHelper.GetItemsName(request.ExecutorId),
                        ResponsibleId = DictionaryHelper.GetItemId(request.ExecutorId.FirstOrDefault()),
                        ResponsibleValue = DictionaryHelper.GetItemName(request.ExecutorId.FirstOrDefault()),
                ...
                LogHelper.Log.Error("Не удалось выполнить MultiReassignment", ex);
                return Json(new { State = false });
            }
        ...
        public ActionResult GetReassinmentTaskList([DataSourceRequest] DataSourceRequest request, string ids) {
            if (string.IsNullOrEmpty(ids))
                return null;
            var tasksId = ids.Split(',').Select(Guid.Parse);
            var tasks = _db.Tasks.Include(o => o.Document).Where(o =>
            ...
            var list = tasks.Select(x => new MultiReassignmentDetailViewModel {
                Id = x.Id,
                DocumentId = x.DocumentId,
                Number = x.Document.Number,
                Date = x.Document.DocumentDate,
                OutgoingNumber = x.Document.OutgoingNumber,
                OutgoingDate = x.Document.OutgoingDate,
                CorrespondentsValue = x.Document.CorrespondentsValue,
            });

            var response = new List<MultiReassignmentDetailViewModel>();
            foreach (var item in list){
                item.ProxyOrganizationName = DocumentHelper.GetProxyOrganizationName(_db, item.DocumentId.Value);
                response.Add(item);
            }
        ...
        public ActionResult Resolution(Guid taskId)
        {
            ...
            TaskAction action = new TaskAction()
            {
                ...
                Type = task.Type,
                State = task.State,
                ExecutionDate = task.ExecutionDate,//task.Document.ExecutionDate,
                DocumenetDate = task.Document.DocumentDate.HasValue ? task.Document.DocumentDate.Value.ToString("dd.MM.yyyy") : string.Empty
            };
        ...
        private void SendPriceProjectNotification(Document document, int? remarkId) {
            ...
            var remarkType = _db.RemarkTypes.FirstOrDefault(x => x.Id == remarkId);
            if (pp != null) {
                //Смена статуса только для замечания и замечания по вложению
                if (remarkType!=null && (remarkType.Code.Equals("1")|| remarkType.Code.Equals("3"))) {
        ...
        public ActionResult SigningDoc(Guid documentId)
        {
            ...
            TaskAction action = new TaskAction{
                ...
                Type = 3,
                State = 0,
                ExecutionDate = document.ExecutionDate,//DateTime.Now,
        ...
        public ActionResult Withdraw(Guid taskId, Guid parentModel) {
            Task currentTask = _db.Tasks.Include("Document").FirstOrDefault(o => o.Id == taskId);
            if (currentTask != null) {
                ViewBag.TaskDocumentNumber = currentTask.Document.Number;
                ViewBag.TaskDocumentDate = currentTask.Document.DocumentDate;
            }
            ViewBag.ParentModel = parentModel;
            return PartialView(taskId);
        }

        public ActionResult WithdrawConfirm(string id, string text){
            try {
                var Id = new Guid(id);
                var mainTask = _db.Tasks.FirstOrDefault(x => x.Id == Id && x.Type == 0);

                //Когда отзывает руководитель(автор задачи)
                if (mainTask != null){
                    var childTasks = _db.Tasks.Where(x =>
                        x.DocumentId == mainTask.DocumentId
                        //&& x.AuthorId == mainTask.AuthorId
                        && x.Type == 2
                        && (
                            x.State == 0
                            || x.State == 4
                ...
                //Когда Возвращает сам исполнитель
                var innerTask = _db.Tasks.FirstOrDefault(x => x.Id == Id && x.Type == 2);
                if (innerTask != null) {
                    var baseTask = _db.Tasks.FirstOrDefault(x =>
                         x.DocumentId == innerTask.DocumentId
                        //&& x.AuthorId == innerTask.AuthorId
                        && x.Type == 0);
                    if (baseTask != null) {
                        baseTask.State = 0;
                        if (!string.IsNullOrEmpty(text))
                            baseTask.Text = text;
                        _db.Tasks.Remove(innerTask);

                        _db.SaveChanges();
                    }
                    else {
                        LogHelper.Log.Error("Не удалось найти базовую задачу. Выход");
                        return Json(new { State = false });
                    }
                ...
                //Type 0 State 1
                //find by AuthorId, DocumentId where type=2 - Delete task
                //update State from 1 to 0

                //Type 2 State all
                //find by AuthorId, DocumentId where type=0 and state=1 - Update state to 0
                //delete by taskId

                return Json(new { State = true });
            }
            catch (Exception ex) {
                LogHelper.Log.Error("Ошибка при отзыве задачи", ex);
                return Json(new { State = false });
            }
