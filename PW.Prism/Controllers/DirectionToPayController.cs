...
namespace PW.Prism.Controllers
{
    public class DirectionToPayController : Controller
    {
        ...
        public JsonResult ReadDirectionList([DataSourceRequest] DataSourceRequest request)
        {
            ...
            var data = repository.GetDirectionToPaysViews (d => d.CreateEmployeeId == currentUserId || d.ExecutorId == currentUserId);
        ...
        public JsonResult CreateDirection(EXP_DirectionToPaysView model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                repository.Insert(m);
                //repository.Save();
                if (model.Type == EXP_DirectionToPaysView.ExpertWorkType)
                {
        ...
        public JsonResult ReadDrugDeclarationList([DataSourceRequest] DataSourceRequest request, int type)
        {
            ...
            var curentUserId = UserHelper.GetCurrentEmployee().Id;
            var data = repository.GetDrugDeclarations(dd => dd.Number != null 
                && !dd.EXP_DirectionToPays.Any(d => d.Type == type && d.DeleteDate == null)
                && dd.EXP_ExpertiseStage.Any(es => es.EXP_DIC_Stage.Code == EXP_DIC_Stage.PrimaryExp)
                && dd.EXP_ExpertiseStage.Any(es => es.EXP_ExpertiseStageExecutors.Any(ese => ese.ExecutorId == curentUserId)))
            .Include(dd => dd.EXP_DIC_Type );
        ...
        public ActionResult GetAgreementList(Guid directionId)
        {
            ...
            var tasks = manager.GetActiveTasks(directionId, Dictionary.ExpActivityType.DirectionToPayAgrement);
