...
{
	[Authorize]
    public class ProtocolDocController : Controller
    {
		...
		public ActionResult ListDocument([DataSourceRequest] DataSourceRequest request, int? status, int? monitoring) {
			var docs = db.Documents.Where(o => o.IsDeleted == false && o.DocumentType == 3 && o.ProjectType == 4);
			if (status.HasValue) {
				docs = docs.Where(o => o.StateType == status);
			}
			if (monitoring.HasValue) {
				docs = docs.Where(o => o.MonitoringType == monitoring);
			}
