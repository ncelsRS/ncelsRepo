...
namespace PW.Prism.Controllers
{
    ...
    public class ProjectController : Controller
    {
        ...
        public ActionResult RequestOrderBaseReport(string reportName){
            Guid guid = Guid.NewGuid();
        ...
        public ActionResult RequestOrderBaseReportGenerate(string id, string reportName, int exportType){
            if (!string.IsNullOrEmpty(id)) {
                ...
                report.Load(Server.MapPath("../Reports/List/" + reportName + ".mrt"));

                foreach (var data in report.Dictionary.Databases.Items.OfType<StiSqlDatabase>()){
                    data.ConnectionString = UserHelper.GetCnString();
                }

                if (reportName.Equals("RequestOrderAllRows")) {
                    //Если выбран конкретный приказ
                    if (param.Length == 3){
                        report.Dictionary.Variables["QueryType"].ValueObject = 1;
                        report.Dictionary.Variables["RoId"].ValueObject = param[2];
                    ...
                    //Если выбран год
                        ...
                        //Если тип Другие
                        if (type == 4) {
                            report.Dictionary.Variables["QueryType"].ValueObject = 3;
                            report.Dictionary.Variables["ListType"].ValueObject = type;
                            report.Dictionary.Variables["ListYear"].ValueObject = year;
                        }
                        else {
                            report.Dictionary.Variables["QueryType"].ValueObject = 2;
                            report.Dictionary.Variables["ListType"].ValueObject = type;
                            report.Dictionary.Variables["ListYear"].ValueObject = year;
                        }
                ...
                else if (reportName.Equals("RequestOrderStatistic")) {
                    //Если выбран конкретный приказ
                    if (param.Length == 3){
                        report.Dictionary.Variables["QueryType"].ValueObject = 1;
                        report.Dictionary.Variables["OrderId"].ValueObject = param[2];
                    ...
                    //Если выбран год
                        ...
                        if (type == 4) {
                            report.Dictionary.Variables["QueryType"].ValueObject = 3;
                            report.Dictionary.Variables["OrderType"].ValueObject = type;
                            report.Dictionary.Variables["OrderYear"].ValueObject = year;
                        }
                        else {
                            report.Dictionary.Variables["QueryType"].ValueObject = 2;
                            report.Dictionary.Variables["OrderType"].ValueObject = type;
        ...
        public ActionResult GenerateTaskRequestReport() {
            Employee currentEmployee = UserHelper.GetCurrentEmployee();

            StiReport report = new StiReport();
            report.Load(Server.MapPath("../Reports/List/TaskRequestsRoAllRows.mrt"));

            foreach (var data in report.Dictionary.Databases.Items.OfType<StiSqlDatabase>()){
                data.ConnectionString = UserHelper.GetCnString();
            }
            report.Dictionary.Variables["EmployeeId"].ValueObject = currentEmployee.Id;
		...
		public ActionResult PpHistoryDetails(Guid? id){
            ...
            PriceProjectArchiveDetail project = db.PriceProjectArchiveDetails.FirstOrDefault(x=>x.Id==id);
			if (project == null){
				return HttpNotFound();
			}
        ...
        public ActionResult PpHistoryFiles(Guid id){
            return PartialView(id);
        }

        public ActionResult AttachRemarkWindow(Guid attachDicId, Guid ppId) {
            var pp = db.PriceProjects.FirstOrDefault(x => x.Id == ppId);
            var dic = db.Dictionaries.FirstOrDefault(x => x.Id == attachDicId);
            if (pp != null && dic!=null) {
                ViewBag.HasError = false;
                ViewBag.AttachName = dic.Name;
                var remark =
                    db.PP_AttachRemarks.FirstOrDefault(
                        x => x.PriceProjectId == ppId && x.AttachPriceDicId == attachDicId);
                ViewBag.Remark = remark != null ? remark.Note : "";
                ViewBag.PpId = ppId;
                return PartialView(attachDicId);
            }
        ...
        [HttpPost]
        public ActionResult AttachRemarkSave(Guid attDicId, Guid ppId, string note) {
            try {
                var remark = db.PP_AttachRemarks.FirstOrDefault(x => 
                    x.PriceProjectId == ppId 
                    && x.AttachPriceDicId == attDicId);
                if (remark != null) {
                    if (!string.IsNullOrEmpty(note)) {
                        ...
                        db.PP_AttachRemarks.Remove(remark);
                    }
                }
                else {
                    if (!string.IsNullOrEmpty(note)) {
                        remark = new PP_AttachRemarks {
                            Id = Guid.NewGuid(),
                            PriceProjectId = ppId,
                            AttachPriceDicId = attDicId,
                            Note = note,
                            CreatedDate = DateTime.Now
                        };
                        db.PP_AttachRemarks.Add(remark);
                    }
                }
                ...
                LogHelper.Log.Error("Не удалось добавить замечание",ex);
                return Content(bool.FalseString);
            }
        }

        [HttpPost]
        public ActionResult AttachRemarkDelete(Guid id) {
            try {
                var remark = db.PP_AttachRemarks.FirstOrDefault(x => x.Id == id);
                if (remark != null) {
                    db.PP_AttachRemarks.Remove(remark);
                    db.SaveChanges();
                }
                ...
                LogHelper.Log.Error("Не удалось удалить замечание",ex);
                return Content(bool.FalseString);
            }
        }

        public JsonResult AttachRemarkList([DataSourceRequest] DataSourceRequest request, Guid ppId){
            var list = db.PP_AttachRemarksView.Where(x=>x.PriceProjectId == ppId).ToList();
