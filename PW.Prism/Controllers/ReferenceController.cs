...
namespace PW.Prism.Controllers
{
    ...
    public class ReferenceController : Controller
    {
        ...
        public ActionResult ReadRemarkTypes(){
            var data = db.RemarkTypes.Select(x => new {x.Code, x.Name})
                .OrderBy(x => x.Name);

            return Json(data.ToList().Select(m => new { Id = m.Code, m.Name}), JsonRequestBehavior.AllowGet);
        }
        ...
        public ActionResult List(string text)
        {
            if (text == "Nomenclature")
            {
                ...
                var data =
                    db.Dictionaries.Where(
                        ...
                        .ThenBy(o => o.Code)
                        .ToList();
                if (!EmployeePermissionHelper.IsAllNomenclature)
                {
            ...
            var dataDic = db.Dictionaries.Where(o => o.Type == text && o.ExpireDate == null).OrderBy(o => o.Name).ToList();
        ...
        {
            var data = db.Dictionaries.Where(o => o.Type == text);
            if (Id != null)
                data = data.Where(o => o.Id != Id);

            var dataDic = data.OrderBy(o => o.Name).ToList();
            return Json(dataDic.Select(o => new { o.Id, Name = LocalizationHelper.GetString(o.Name, o.NameKz) }),
                JsonRequestBehavior.AllowGet);
        }
        ...
        {
            Guid organizationId = UserHelper.GetCurrentEmployee().OrganizationId;
            if (EmployeePermissionHelper.IsAllEmploye)
            {
        ...
        public ActionResult ListEmployeeByUserDivision()
        {
            var userId = UserHelper.GetCurrentEmployee().Id;
            var divisionId = db.Employees.Where(e => e.Id == userId).Select(e => e.Position.ParentId).FirstOrDefault();
            return Json(
                db.Employees.Where(e => e.Position.ParentId == divisionId).Select(o => new {o.Id, Name = o.DisplayName}),
                JsonRequestBehavior.AllowGet);
        }
        ...
        public ActionResult AvgExchangeRates() {
            return PartialView(Guid.NewGuid());
        }

        public ActionResult AvgExchangeRateRead([DataSourceRequest] DataSourceRequest request){
            var data = db.PP_DIC_AvgExchangeRateView
                .OrderBy(o => o.Id);
            return Json(data.ToDataSourceResult(request));
        }

        [AcceptVerbs(HttpVerbs.Post)]
        public ActionResult AvgExchangeRateCreate([DataSourceRequest] DataSourceRequest request, Guid CurrencyId, decimal Rate){
            if (CurrencyId != null && Rate > 0) {
                var id = Guid.NewGuid();
                var exchangeRate = new PP_DIC_AvgExchangeRate {
                    Id = id,
                    ChangeDate = DateTime.Now,
                    ChangeEmployeeId = UserHelper.GetCurrentEmployee().Id,
                    CurrencyId = CurrencyId,
                    Rate = Rate
                };

                db.PP_DIC_AvgExchangeRate.Add(exchangeRate);
                db.SaveChanges();
                var row = db.PP_DIC_AvgExchangeRateView.FirstOrDefault(x => x.Id == id);

                return Json(new[] { row }.ToDataSourceResult(request, ModelState));
            }
            throw new Exception("Не указан курс");
        }

        [AcceptVerbs(HttpVerbs.Post)]
        public ActionResult AvgExchangeRateUpdate([DataSourceRequest] DataSourceRequest request, Guid Id, Guid CurrencyId, decimal Rate){
            if (Id != null && CurrencyId != null && Rate > 0){
                var d = db.PP_DIC_AvgExchangeRate.First(x => x.Id == Id);
                d.ChangeDate = DateTime.Now;
                d.ChangeEmployeeId = UserHelper.GetCurrentEmployee().Id;
        ...
        [AcceptVerbs(HttpVerbs.Post)]
        public ActionResult AvgExchangeRateDestroy([DataSourceRequest] DataSourceRequest request, Guid Id) {
            var d = db.PP_DIC_AvgExchangeRate.First(x => x.Id == Id);
            db.PP_DIC_AvgExchangeRate.Remove(d);
            db.SaveChanges();

            return Json(new[] { d }.ToDataSourceResult(request, ModelState));
        }

        public ActionResult GetCurrencies() {
            var currencies = db.Dictionaries.Where(x=>x.Type.Equals("Currency")).Select(x=> new {
                x.Id,
                x.Name
            }).OrderBy(t => t.Name);
        ...
        public ActionResult PrismRejectReasons() {
            var currencies = db.Dictionaries.Where(x => x.Type.Equals("PrismRejectReason")).Select(x => new {
                x.Id,
                x.Name
            }).OrderBy(t => t.Name);
