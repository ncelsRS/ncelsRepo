...
namespace PW.Prism.Controllers
{
    ...
    public class UploadController : Controller
    {
        ...
        public ActionResult Download(string id, string name){
            ...
            return File(dataDownload, System.Net.Mime.MediaTypeNames.Application.Octet, name);
        }

        public ActionResult Delete(string id, string name) {
            FileHelper.DeletePreview(id, name);
            return Json(FileHelper.DeleteAttach(id, "", name), JsonRequestBehavior.AllowGet);
        }

        [HttpPost]
        public ActionResult UploadPngData(string id, string data) {
            var bytes = Convert.FromBase64String(data);
            var fileName = string.Format("screen{0}.png", DateTime.Now.ToString("HHmmssffff"));
            var result = FileHelper.UploadFile(bytes, id, fileName);
            return Json(result ? fileName : "", JsonRequestBehavior.AllowGet);
        }
        ...
        public FileResult FileDownload(string id = null, string path = null, string name = null, string fileId = null)
        {
            ...
            if (!string.IsNullOrEmpty(fileId))
            {
                ...
                var fileLink = db.FileLinks.FirstOrDefault(e => e.Id == fileKey);
                ...
                file =
                    FileHelper.GetFilePath(
                        ...
                        null, fileKey + Path.GetExtension(fileLink.FileName));
