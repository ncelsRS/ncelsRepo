...
namespace PW.Prism.Controllers
{
    ...
    public class ContractController : Controller
    {
       ...
       {        new Item { Id = "0", Name = "Новый"},
        ...
        {
            IQueryable<ContractJournal> query = db.ContractJournals.AsNoTracking().Where(e => e.ContractId == null && e.StatusCode!=Ncels.Database.DataModel.Contract.StatusNew);
            var contractSettings = _contractRepository.GetContractProcSetting();
            var currentEmployee = UserHelper.GetCurrentEmployee();
        ...
        public ActionResult ContractCard(Guid contractId)
        {
            ...
            var contract =  db.Contracts.AsNoTracking().FirstOrDefault(e => e.Id == contractId);
            var vm = new ContractViewModel()
            {
                ...
                IsContractWithoutDc = !(contract.IsHasDigCert==true),
        ...
        public PartialViewResult ContractDetails(Guid contractId, string parentUiId)
        {
            ...
            contract.Agent = db.OrganizationsViews.FirstOrDefault(m => m.Id == contract.ApplicantContract.AgentOrganizationId) ?? new OrganizationsView();
        ...
        public ActionResult AgreementHistoryList([DataSourceRequest] DataSourceRequest request, Guid contractId)
        {
            var items = _contractRepository.GetContractAgreementHistory(contractId);
        ...
        public ActionResult Reject(Guid contractId)
        {
            ViewBag.UiId = Guid.NewGuid().ToString();
            ViewBag.Reject = true;
            return PartialView("Approve", contractId);
        }
        ...
        [HttpGet]
        public ActionResult SetContractNumber(Guid contractId)
        {
            ViewBag.ContractId = contractId;
            ViewBag.Contract = _contractRepository.GetContractById(contractId);
            return PartialView("SetContractNumber", Guid.NewGuid());
        }

        [HttpPost]
        public ActionResult SetContractNumber(Guid contractId, string contractNumber, DateTime contractDate)
        {
            new ContractRepository(false).SetNumberAndDate(contractId, contractNumber, contractDate);
            return Json("OK", JsonRequestBehavior.AllowGet);
        }
    }
}