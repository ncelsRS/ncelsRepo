...
namespace PW.Prism.Controllers
{
    ...
    public class ApplicationController : Controller
    {
     ...
     {        new Item { Id = "0", Name = "Новый"},
        ...
        public ActionResult FormPriceCalculating(Guid id) {
            return PartialView(id);
        }
        public ActionResult FormPriceMarkup(Guid id) {
            var currentPrice = new PriceProjectRepository().GetCurrentPrice(id);
            ViewBag.NcelsPrice = 0;
            ViewBag.RfkPrice = 0;
            ViewBag.OptPrice = 0;
            ViewBag.RznPrice = 0;
            var pp = _db.PriceProjects.FirstOrDefault(x => x.Id == id);
            if (currentPrice != null) {
                if (pp != null && pp.Status == (int)PriceProjectStatus.Finished){
                    ViewBag.NcelsPrice = currentPrice.UnitPrice;
                }
                ViewBag.RfkPrice = currentPrice.FixRfkPrice;
                ViewBag.OptPrice = currentPrice.MarkupCostOpt;
                ViewBag.RznPrice = currentPrice.MarkupCost;
            }
        ...
        public ActionResult FormPrice(Guid id){
            ...
            Price project = null;
            if (projectRkk.Type == (int)PriceProjectType.PriceLs){
                project = _db.Prices.FirstOrDefault(x => x.PriceProjectId == id
                    && x.Type == (int)PriceType.LsCurrentPrice);
            }
            else if (projectRkk.Type == (int)PriceProjectType.RePriceLs){
                project = _db.Prices.FirstOrDefault(x => x.PriceProjectId == id
                    && x.Type == (int)PriceType.ReLsCurrentPrice);
            }
            else{
                var avgImnCurrentPrice = _db.Prices.FirstOrDefault(x => x.PriceProjectId == id && x.Type == (int)PriceType.AvgImnCurrentPrice);
                //Если еще нет общей цены
                if (avgImnCurrentPrice == null) {
                    List<Price> projectImn = new List<Price>();
                    if (projectRkk.Type == (int) PriceProjectType.PriceImn) {
                        projectImn = _db.Prices.Where(x => x.PriceProjectId == id
                            && x.Type == (int)PriceType.ImnCurrentPrice).ToList();
                    }
                    else if (projectRkk.Type == (int)PriceProjectType.RePriceImn){
                        projectImn = _db.Prices.Where(x => x.PriceProjectId == id
                            && x.Type == (int)PriceType.ReImnCurrentPrice).ToList();
                    }
                ...
                //Если общая цена уже сформирована
                else {
                    project = avgImnCurrentPrice;
                }
            ...
            if (project != null && projectRkk != null)
            {
                ...
                calcModel.AvgRozPriceNote = project.AvgRozPriceNote;
                calcModel.AvgOptPriceNote = project.AvgOptPriceNote;
                calcModel.AvgRozPriceLink = project.AvgRozPriceLink;
                calcModel.AvgOptPriceLink = project.AvgOptPriceLink;
            ...
            // для заголовка на странице расчета цены
            if(projectRkk.Type == (int)PriceProjectType.PriceLs || projectRkk.Type == (int)PriceProjectType.RePriceLs) {
                ViewBag.LsTitle = $"{(!string.IsNullOrEmpty(projectRkk.NameRu) ? projectRkk.NameRu : "")}{(!string.IsNullOrEmpty(projectRkk.FormNameRu) ? string.Concat("; ", projectRkk.FormNameRu) : "")}{(!string.IsNullOrEmpty(projectRkk.Dosage) ? string.Concat("; ", projectRkk.Dosage) : "")}{(!string.IsNullOrEmpty(projectRkk.CountPackage) ? string.Concat("; №", projectRkk.CountPackage) : "")}{(!string.IsNullOrEmpty(projectRkk.Concentration) ? string.Concat("; ", projectRkk.Concentration) : "")}{(!string.IsNullOrEmpty(projectRkk.RegNumber) ? string.Concat("; ", projectRkk.RegNumber) : "")}";
            } else if (projectRkk.Type == (int) PriceProjectType.PriceImn ||
                       projectRkk.Type == (int) PriceProjectType.RePriceImn)
            {
                ViewBag.LsTitle = projectRkk.NameRu;
            }
        ...
        public ActionResult ListPrice([DataSourceRequest] DataSourceRequest request, Guid id)
        {
            ...
            if (docs.Count == 0) {
                ...
                CreatePriceLands(id, "Британский национальный формуляр");
                docs =
                    _db.PricesViews.Where(o => o.PriceProjectId == id && o.Type == 99)
                        .OrderBy(x => x.CreatedDate)
                        .ToList();
            }
            else {
                if (docs.FirstOrDefault(x => x.CountryName != null && x.CountryName.Equals("Британский национальный формуляр")) == null) {
                    CreatePriceLands(id, "Британский национальный формуляр");
                    docs = _db.PricesViews.Where(o => o.PriceProjectId == id && o.Type == 99)
                            .OrderBy(x => x.CreatedDate)
                            .ToList();
                }
            }
        ...
        public ActionResult PriceCreate([DataSourceRequest] DataSourceRequest request, PricesViewModel dictionary, Guid id)
        {
            ...
            cost = GetExchangeCost(cost, dictionary.ManufacturerPriceCurrencyDicId);
        ...
        private decimal GetExchangeCost(decimal cost, Guid? currencyId) {
            if (!currencyId.HasValue) {
                LogHelper.Log.Error("Не указана валюта");
                return cost;
            }
            try {
                var avgRate = _db.PP_DIC_AvgExchangeRate.FirstOrDefault(x => x.CurrencyId.Equals(currencyId.Value));
                if (avgRate != null) {
                    return cost * avgRate.Rate;
                }
                LogHelper.Log.Error("По указанной валюте нет значения курса валют. Пробуем альтернативный сценарий рассчета");
                return AlternativeExchangeCost(cost, currencyId.Value);
            }
            catch (Exception ex) {
                LogHelper.Log.Error("Не удалось рассчитать стоимость по курсу валюты", ex);
                return cost;
            }
        }

        private decimal AlternativeExchangeCost(decimal cost, Guid currencyId) {
            try {
                DateTime now = DateTime.Now.Date;
                var valutaPw = _db.Dictionaries.FirstOrDefault(x => x.Id == currencyId);
                if (valutaPw != null){
                ...
                LogHelper.Log.Error("Не удалось рассчитать стоимость по курсу валюты в AlternativeExchangeCost", ex);
                return cost;
            }
        }
        ...
        public ActionResult PriceUpdate([DataSourceRequest] DataSourceRequest request, PricesViewModel dictionary)
        {
            ...
            price.ManufacturerPriceNote = dictionary.ManufacturerPriceNote;
            price.RefPrice = cost;
        ...
        [HttpPost]
        public ActionResult DocumentSave(CalcModel model) {
            Price project = null;
            //Если ЛС - все по-старому
            if (model.PriceProjectType == (int)PriceProjectType.PriceLs){

                project = _db.Prices.FirstOrDefault(x =>
                    x.PriceProjectId == model.Id
                    && x.Type == (int)PriceType.LsCurrentPrice
                );
            }
            else if (model.PriceProjectType == (int)PriceProjectType.RePriceLs){
                project = _db.Prices.FirstOrDefault(x =>
                    x.PriceProjectId == model.Id
                    && x.Type == (int)PriceType.ReLsCurrentPrice
                );
            ...
            {
                //Для ИМН ищем общую цену
                project = _db.Prices.FirstOrDefault(x =>
                    x.PriceProjectId == model.Id
                    && x.Type == (int)PriceType.AvgImnCurrentPrice
                );
                //Если общей цены еще нет - создаем
                if (project == null){
                    project = new Price{
                        Id = Guid.NewGuid(),
                        PriceProjectId = model.Id,
                        Type = (int)PriceType.AvgImnCurrentPrice,
                        CipPrice = model.CipPrice,
                        ManufacturerPrice = model.ManufacturerPrice,
                        OwnerPrice = model.OwnerPrice,
                        RefPrice = model.RefPrice,
                        UnitPrice = model.UnitPrice,
                        BritishPrice = model.BritishPrice,
                        CreatedDate = model.CreatedDate,
                        CalcDateStart = model.CalcDateStart,
                        CalcDateEnd = model.CalcDateEnd,
                        MarkupCost = model.MarkupCost,
                        MarkupCostOpt = model.MarkupCostOpt
                    };
                    _db.Prices.Add(project);
                }
            }
            ...
            if (project != null){
                ...
                project.AvgRozPriceNote = model.AvgRozPriceNote;
                project.AvgOptPriceNote = model.AvgOptPriceNote;
                project.AvgRozPriceLink = model.AvgRozPriceLink;
                project.AvgOptPriceLink = model.AvgOptPriceLink;
        ...
        private decimal CalcMarkupOpt(Price obj) {
            return CalcMarkupOpt(obj.UnitPrice);
        }

        private decimal CalcMarkupOpt(decimal price){
            if (price <= 300)
                return price + price * (decimal)0.14;

            if (price > 300 && price <= 3000)
                return price + price * (decimal)0.13;

            if (price > 3000 && price <= 30000)
                return price + price * (decimal)0.12;

            if (price > 30000 && price <= 100000)
                return price + price * (decimal)0.11;

            return price + price * (decimal)0.10;
        }

        private decimal CalcMarkupRzn(Price obj) {
            return CalcMarkupRzn(obj.UnitPrice);
        }

        private decimal CalcMarkupRzn(decimal price){
            if (price <= 300)
                return price + price * (decimal)0.24;

            if (price > 300 && price <= 3000)
                return price + price * (decimal)0.23;

            if (price > 3000 && price <= 30000)
                return price + price * (decimal)0.22;

            if (price > 30000 && price <= 100000)
                return price + price * (decimal)0.21;

            return price + price * (decimal)0.20;
        }

        public ActionResult GetMarkupPrice(Guid id, string rfkPrice){
            var _rfkPrice = decimal.Parse(rfkPrice.Replace(".", ","));

            var pp = _db.PriceProjects.FirstOrDefault(x => x.Id == id);
            if (pp != null) {
                Price currentPrice = null;
                //Рассматриваем ЛС
                if (pp.Type == (int) PriceProjectType.PriceLs) {
                    currentPrice = _db.Prices.FirstOrDefault(x =>
                        x.PriceProjectId == id
                        && x.Type == (int) PriceType.LsCurrentPrice);
                }
                else if (pp.Type == (int)PriceProjectType.RePriceLs){
                    currentPrice = _db.Prices.FirstOrDefault(x =>
                        x.PriceProjectId == id
                        && x.Type == (int)PriceType.ReLsCurrentPrice);
                }
                //Рассматриваем ИМН
                    ...
                    currentPrice = _db.Prices.FirstOrDefault(x =>
                        x.PriceProjectId == id
                        && x.Type == (int)PriceType.AvgImnCurrentPrice);
                    //Если общей цены нет - ругаемся
                    if (currentPrice == null) {
                        throw new Exception("Не удалось определить текущую цену ИМН. Необходимо сначала произвести рассчет");
                    }
                }

                if (currentPrice != null) {
                    currentPrice.FixRfkPrice = _rfkPrice;
                    currentPrice.MarkupCost = CalcMarkupRzn(_rfkPrice);
                    currentPrice.MarkupCostOpt = CalcMarkupOpt(_rfkPrice);
                    _db.Entry(currentPrice).State = EntityState.Modified;
                    _db.SaveChanges();

                    return Json(new { optMarkup = currentPrice.MarkupCostOpt, rznMarkup = currentPrice.MarkupCost }, JsonRequestBehavior.AllowGet);
                }
                else {
                    throw new Exception("Так и не удалось определить текущую цену");
                }
            }

            throw new Exception("Не удалось найти заявление по идентификатору");
        }
    }
}
