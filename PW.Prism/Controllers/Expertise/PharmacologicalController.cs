...
namespace PW.Prism.Controllers.Expertise
{
    ...
    /// </summary>
    public class PharmacologicalController : DrugPrimaryController
    {
        ...
        {
            return CodeConstManager.STAGE_PHARMACOLOGICAL;
        }
        [HttpPost]
        public override ActionResult CloneDosageFinalDoc(Guid dosageId)
        {
            new DrugPharmacologicalRepository().CloneDosageFinalDoc(dosageId);
            return Json(new { Success = true });
        }
        [HttpPost]
        public override ActionResult UpdateFinalDocument(string fieldName, string fieldValue, Guid objectId)
        {
            var finalDoc = new DrugPharmacologicalRepository().UpdateFinalDocument(fieldName, fieldValue, objectId, UserHelper.GetCurrentEmployee().Id);
