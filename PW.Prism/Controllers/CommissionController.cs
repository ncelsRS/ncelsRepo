...
namespace PW.Prism.Controllers
{
    ...
    public class CommissionController : Controller
    {
        ...
        {
            var types = GetUserCommissionAccessTypes();
            var dbCommissions = db.CommissionsViews.Where(x => types.Contains(x.TypeId));
            var data = dbCommissions.Select(x => new CommissionListItemModel
            {
                Id = x.Id,
                Number = x.FullNumber,
        ...
        private static readonly string ConstDepartmentExpCode = "ES-DEPARTMENT"; // 'Департамент специализированной экспертизы ЛС'
        private static readonly string ConstDepartmentExpFmcCode = "FMC-DEPARTMENT"; // 'Управление фармацевтической экспертизы'
        private static readonly string ConstDepartmentExpFmkCode = "FMK-DEPARTMENT"; // 'Управление фармакологической экспертизы'

        private static List<int> GetUserCommissionAccessTypes()
        {
            var dep = UserHelper.GetDepartment();
            var types = new List<int>();
            types.Add((int) PW.Ncels.Database.Enums.CommissionType.ExpertCouncil);
            if (dep.Code == ConstDepartmentExpCode)
            {
                types.Add((int) PW.Ncels.Database.Enums.CommissionType.PharmaceuticalCommission);
                types.Add((int) PW.Ncels.Database.Enums.CommissionType.PharmacologicalCommission);
            }
            else if (dep.Code == ConstDepartmentExpFmkCode)
            {
                types.Add((int) PW.Ncels.Database.Enums.CommissionType.PharmacologicalCommission);
            }
            else if (dep.Code == ConstDepartmentExpFmcCode)
            {
                types.Add((int) PW.Ncels.Database.Enums.CommissionType.PharmaceuticalCommission);
            }
            return types;
        ...
        public ActionResult Commission(int? id)
        {
            ...
            {
                var types = GetUserCommissionAccessTypes();
                model.TypeList = db.CommissionTypes.Where(x => types.Contains(x.Id)).Select(x => new SelectListItem { Value = x.Id.ToString(), Text = x.Name }).ToList();
            }
        ...
        public ActionResult CommissionDrugDelaretionRead([DataSourceRequest] DataSourceRequest request, int commissionId)
        {
            ...
            {
                var dbStageIds = db.EXP_ExpertiseStage.Where(x => x.EXP_ExpertiseStageExecutors.Any(x2 => x2.ExecutorId == emp.Id)).Select(x=>x.Id);
        ...
        public ActionResult RemoveCommissionDrugDeclaration(int id)
        {
            ...
            var ncDosage = db.CommissionDrugDosageNeedCommissions
                .FirstOrDefault(x => x.DrugDosageId == dbComQuestion.DrugDosageId && x.StageId == dbComQuestion.StageId);
            if (ncDosage != null)
            {
                if (dbCommission.TypeId == (int)PW.Ncels.Database.Enums.CommissionType.ExpertCouncil)
                {
