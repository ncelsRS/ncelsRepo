...
namespace PW.Prism.Controllers.PriceProject
{
    public class ProtocolController : Controller{
        ...
        [HttpPost]
        public ActionResult UploadProtocolFile(IEnumerable<HttpPostedFileBase> files, string protocolId) {
            try {
                if (files != null && !string.IsNullOrEmpty(protocolId)) {
                    var file = files.FirstOrDefault();
                    if (file != null) {
                        var id = new Guid(protocolId);
                        var pp = db.PP_Protocols.FirstOrDefault(x => x.Id == id);
                        if (pp != null) {
                            FileHelper.DeleteProtocolFiles(protocolId);
                            UploadHelper.UploadProtocolDocument(file.InputStream, file.FileName, protocolId);
                        }
                    }
                }
                ...
                LogHelper.Log.Error("Ошибка сохранения файла протокола", ex);
                return Content(bool.FalseString);
            }
        }

        [HttpPost]
        public ActionResult RemoveProtocolFile(string[] fileNames, string protocolId) {
            try {
                var id = new Guid(protocolId);
                ...
                LogHelper.Log.Error("Ошибка удаления файла протокола", ex);
                return Content(bool.FalseString);
            }
        }
