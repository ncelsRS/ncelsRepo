...
function InitializeActRecepttionFile(name) {
    ...
    function loadDocument() {
        ...
        $.ajax({
            type: 'get',
            url: '/OBKExpDocument/DocumentRead?Id=' + name,
...
function InitializeSerialActRecepttionFile(name, filePath)
{
    var validator = $("#outDocForm" + name).kendoValidator().data("kendoValidator");
    var viewModel = kendo.observable({
        document: {},
    ...
    function loadDocument()
    {
        ...
        $.ajax({
            type: 'get',
            url: '/SafetyAssessment/DocumentReadSeries?filePath=' + filePath,
            contentType: 'application/json; charset=utf-8',
            success: function (result)
            {
                result = JSON.parse(result);
