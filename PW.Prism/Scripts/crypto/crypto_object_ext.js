...
function crypt_object_init(callbackM, uiId) {
    _digSignUiId = uiId;
    if (webSocket === null || webSocket.readyState === 3 || webSocket.readyState === 2) {
        ...
        webSocket.onclose = function (event) {
            ...
            registerLogFile('Error connection - Code: ' + event.code + ' Reason: ' + event.reason);
        };
        ...
        webSocket.onmessage = function (event) {            
            ...
            if (result != null) {
                ...
                if (_callback)
                    window[_callback](rw, _digSignUiId);
            }
            console.log(event);
            if (isSetLimitMin)
...
function openDialog() {
    if (confirm("Ошибка при подключений к NCALayer. Убедитесь что NCALayer запущена и нажмите ОК") === true) {
        //        registerLogFile('openDialog - Ошибка при подключений к NCALayer. Убедитесь что NCALayer запущена и нажмите ОК');
        location.reload();
    }
}
...
function pingLayer() {
    ...
    try {
        ...
        registerLogFile('pingLayer - Closing connection. Reason: ' + e.message);
        webSocket.close();
    }
}

function browseKeyStore(storageName, fileExtension, currentDirectory, callBack, uiId) {
...
function getKeys(storageName, storagePath, password, type, callBack, uiId) {
    var getKeysVar = {
        "method": "getKeys",
...
function signXml(storageName, storagePath, alias, password, xmlToSign, callBack, uiId) {
    var signXmlVar = {
        "method": "signXml",
