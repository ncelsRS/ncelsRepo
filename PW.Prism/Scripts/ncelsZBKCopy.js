function InitFilterZBKCopy(uiId) {
    ...
    $('#toWork' + uiId).click(function () {
        ...
        if (idsToSend.length > 0) {
            ...
            window.data("kendoWindow").refresh('/ZBKCopy/SetExecuter');
            window.data("kendoWindow").center();
            window.data("kendoWindow").open();
        } else {
            grid.dataSource.read();
...
function getZBKCopyDetails(parameters, number, controllerName) {
    ...
    if (element === null) {
        ...
        var content = '<div id="' + parameters + '" style="overflow-y: scroll;"> </div>';
...
function getZBKCopyDetailsOBK(parameters, number, controllerName)
{
    if (docArray.indexOf(parameters.toLowerCase()) !== -1)
        docArray.splice(docArray.indexOf(parameters.toLowerCase()), 1);
    var element = document.getElementById(parameters);
    if (element === null)
    {
        ...
        $.ajax({
            url: "/" + controllerName + "/EditBlanks?id=" + parameters,
            //type: "POST",
            success: function (result)
            {
                // refreshes partial view
