...
function panelTaskTypeSelect(e) {
    var selectType = $(e.item).find("> .k-link").attr('ItemType');
    if (selectType != null) {
        var selectValue = $(e.item).find("> .k-link").attr('ItemId');
        var gridId = $(e.item).find("> .k-link").attr('ModelId');
        var grid = $("#gridTaskList" + gridId).data();
        var filter = new Array();

        if (selectType == "TypeValue") {
            filter.push({ field: "Type", operator: "eq", value: selectValue });
        }
    ...
    $("#find" + name).click(function (e) {
        ...
        if (text != '') {
            ...
            if (findType == 0) {
                ...
                $filter.push({ field: "ProxyOrganizationName", operator: "contains", value: text });
...
function taskOnWithdraw(e, parentModel) {
    //return;
    var window = $("#TaskCommandWindow");
    window.kendoWindow({
        width: "550px",
        ...
        title: 'Отзыв',
        actions: ["Close"],
        content: '/Task/Withdraw?taskId=' + e + '&parentModel=' + parentModel
    });
    ...
    window.data("kendoWindow").refresh('/Task/Withdraw?taskId=' + e + '&parentModel=' + parentModel);
...
function ReassignmentClick(e) {
    var modelId = $("#" + e.sender.element[0].id).attr("modelid");
    var grid = $("#gridTaskList" + modelId).data("kendoGrid");

    var ids = [];
    grid.select().each(function () {
        var dataItem = grid.dataItem($(this));
        ids.push(dataItem.TaskId);
    });
    if (ids.length > 50) {
        ShowAlert("Предупреждение", "Необходимо выбрать не более 50 записей", AlertType.Error);
        return;
    }
    var window = $("#TaskCommandWindow");
    window.kendoWindow({
        ...
        title: 'Распределение',
        actions: ["Close"],
        content: '/Task/MultiReassignment?modelid=' + modelId + '&ids=' + ids.join()
    });
    window.data("kendoWindow").title('Распределение');
    //window.data("kendoWindow").refresh('/Task/MultiReassignment?ids=' + ids.join());
    window.data("kendoWindow").setOptions({
        width: 550,
        height: 'auto'
    });
...
function RequestsReportClick(e) {
    window.open("/Project/GenerateTaskRequestReport");
}

function MultiplySelectionDataBound(arg) {
    //Выделение записи
    $("#" + this.element[0].getAttribute("Id") + " tbody").find(".checkbox").bind("change", function (e) {
        $(e.target).closest("tr").toggleClass("k-state-selected");
    });
    //Вылделение всех записей
    $("#" + this.element[0].getAttribute("Id") + "").find("#checkAll").bind("change", function (e) {
        var checked = e.target.checked;
        var gridId = $(this).parents(".k-grid")[0].id;
        $("#" + gridId + " tbody").find(".checkbox").each(function (ind, item) {
            if (checked) {
...
function MultiplySelectionChange(arg) {
    $("#" + this.element[0].getAttribute("Id") + " tbody").find('tr').find('[type=checkbox]').prop('checked', false);
    $("#" + this.element[0].getAttribute("Id") + " tbody").find('tr.k-state-selected').find('[type=checkbox]').prop('checked', true);
}
