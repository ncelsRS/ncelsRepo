...
function InitializeDataIncomingPrice(name) {
    ...
    PasteImageEvent($("#pasteAvgRznScreenArea"));
    PasteImageEvent($("#pasteAvgOptScreenArea"));
...
function PasteImageEvent(el) {
    var modelId = $(el).attr("model-id");
    var prefix = $(el).attr("prefix");
    var listId = $(el).attr("list-id");
    var docId = modelId + '_' + prefix;
    $(el).on('paste', function(event) {
        ...
        if (blob !== null) {
            kendo.ui.progress($('#loader' + modelId), true);
            ...
            $(reader).on('load', function (e) {
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        $('#' + listId + modelId).data("kendoListView").dataSource.read();
                        $('#' + listId + modelId).data("kendoListView").refresh();
                    ...
                    complete: function() {
                        kendo.ui.progress($('#loader' + modelId), false);
                    }
                });
            });
...
function savePriceData(name) {
    ...
    $.ajax({
        type: 'POST',
        url: '/Application/DocumentSave',
        data: JSON.stringify($("#priceFormToSave" + name).serializeObject()),
        dataType: "json",
        contentType: 'application/json; charset=utf-8',
        success: function (result) {
            ShowAlert("Сообщение","Данные сохранены");
        },
        error: function() {
            ShowAlert("Ошибка", "Не удалось сохранить данные", AlertType.Error);
        },
        complete: function () {
            kendo.ui.progress($('#loader' + name), false);
        }
...
function InitializeDataIncoming(name, repeatId) {
    ...
    var viewModel = kendo.observable({
        ...
        printRef: function (e) {
            e.preventDefault();
            printReportRef(this.get("document.Id"));
        },
        hasBuild: true,
        sendNote: function (e) {
            e.preventDefault();
    ...
    function printReportRef(id) {
        window.open('/Report/PrintRef?Id=' + id);
    }
    ...
    function sendRejectRequest() {
        ...
        window.data("kendoWindow").title('Отклонение заявления');
        window.data("kendoWindow").setOptions({
            width: 550,
            height: 'auto'
        });
        window.data("kendoWindow").refresh('/IncomingDoc/SendRejectRequest?id=' + viewModel.get("document.Id"));
