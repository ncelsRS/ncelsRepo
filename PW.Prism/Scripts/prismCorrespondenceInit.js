...
function InitializeSizeCorrespondenceInit(name) {
    var gridElement = $("#pwContentRightId" + name);
    gridElement.height($(window).height() - 120);
}

function InitializeDataCorrespondenceInit(name, applicantType) {
    ...
    function loadDocument() {
        ...
        $.ajax({
            type: 'get',
            url: '/CorrespondenceDoc/DocumentReadInit?Id=' + name,
            ...
            success: function (result) {
                ...
                if (applicantType == 1) {
                    viewModel.set("document.ApplicantType", 1);
                }
                //viewModel.person = JSON.stringify(result);
                InitializePropertyCorrespondenceInit(name, viewModel);
...
function InitializePropertyCorrespondenceInit(name, viewModel) {
  ...
  // { text: "В работу", value: "0" },
   ...
   { text: "До контроль", value: "4" }
    ...
    $("#ExecutorsNotGuideId" + name).kendoMultiSelect({
        dataTextField: "Name",
        dataValueField: "Id",
     //   maxSelectedItems: 1,
        filter: "startswith",
        ...
        dataSource: {
            transport: {
                read: {
                    url: '/Reference/EmployeeNotGuide'
